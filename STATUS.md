...
| Deploy | `https://runcard.vercel.app` public since 2026-08-28 (Deployment Protection off). **Live = b4491c3** (deployed 2026-08-29 from a clean `git worktree` of HEAD, bundle `index-Y31UbNAm.js`; previous: ff85e2f / `index-USiMctXW.js`, f64789f / `index-CqSj8sfl.js`, 097a01a / `index-D8epXSVf.js`, 46ca5ba / `index-D_Ku5S9V.js`, d604351 / `index-DoMhorJt.js`). Deploy from a worktree whenever another session has uncommitted edits in the working tree; missing manifest → 404, which the loader handles. Project is not git-connected: every deploy is a CLI deploy |
