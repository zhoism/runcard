...
`bun run test` (609) and `bun run build` pass at HEAD. **Environment note (2026-08-29):** with the system Node 20.12 at `/usr/local/bin/node`, `bun run test`/`vite build` fail at startup inside rolldown (`util.styleText` array form needs Node ≥ 22). Run them under bun's runtime instead: `bun --bun x vitest run` and `bun --bun run build`.
