...
| Deploy | `https://runcard.vercel.app` public since 2026-08-28 (Deployment Protection off). **Live = 295bcb5** (deployed 2026-08-29T04:18:25Z via `vercel deploy --prod --yes`, bundle `index-CrhCAa5N.js`; a first attempt returned a transient error, the retry succeeded); missing manifest → 404, which the loader handles. Project is not git-connected: every deploy is a CLI deploy |
