...
export function Spread({ runs, mean, sd, own, ringId, ringWhy }: { runs: IndexEntry[]; mean: number | null; sd: number | null; own?: Owners | null; ringId?: string | null; ringWhy?: string }) {
  ...
  const lanes = laneX.length; const top = 12, step = 17, axis = top + lanes * step + 4, H = axis + 22;
  ...
  return <figure className="spread">
    <svg width="100%" height={H} role="img" aria-label={`ΔG of ${xs.length} runs: ${fmt(min)} to ${fmt(max)} kcal/mol, mean ${fmt(mean)}`}>
      ...
      {sorted.map(r => { const dg = r.delta_g as number; const isRing = r.id === ring; return <a key={r.id} href={`#/run/${r.id}`}>
        ...
        <circle cx={`${X(dg)}%`} cy={cy(lane.get(r.id) as number)} r={isRing ? 6.5 : 6} fill={swatch(r.owner ?? "").fill} stroke={isRing ? "var(--ink)" : "#fff"} strokeWidth={isRing ? 2.5 : 1.5} />
      </a>; })}
      <text x="3%" y={H - 4}>{fmt(min)}</text>
      <text x={`${X(mean)}%`} y={H - 4} textAnchor="middle">mean {fmt(mean)}</text>
      <text x="97%" y={H - 4} textAnchor="end">{fmt(max)}</text>
    </svg>
    <figcaption>One dot per run, ΔG in kcal/mol: {owners.map((o, i) => <span key={o}>{i ? " · " : ""}<span className="sw" style={{ background: swatch(o).fill }} aria-hidden="true" />{nameOf(o)}</span>)}{sd != null ? " · band: mean ± run-to-run SD" : ""}{ring ? <> · ring: <span className="mono">{ring}</span>{ringWhy ? `, ${ringWhy}` : ""}</> : null}</figcaption>
  </figure>;
}
