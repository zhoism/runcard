...
function planSamplingFull(m: Manifest, idx: IndexEntry[], opts: PlanOpts = {}) {
  ...
  const suggested = nstlimNew !== nstlimNow ? {
    ...
    reason: `extend production from ${L0} to ≈ ${lengthForTarget} ps (${extendAlone} steps at dt=${dt}) so this run's within-run corrected frame SEM is projected to reach ±${T} (expected, stationary; frame noise only, not the ensemble target); more matched independent runs is the primary recommendation`,
  ...
  const recommendation = plannedOn == null
    ...
    : targetMet
      ...
      : `expected: ${additional} more independent run${additional === 1 ? "" : "s"} (ig=-1) of ≥ ${Lrec} ps each → n=${nNeeded}${nNeededRange ? ` (plug-in estimate; ±1 SE on the SD gives n = ${nNeededRange.low}–${nNeededRange.high})` : ""}, SEM of the ensemble mean ≈ ${round(semAfter!, 2)} ≤ ${T} kcal/mol. ${lengthForTarget != null ? `Extending this run alone, the within-run corrected frame SEM is projected to reach ±${T} at ≈ ${lengthForTarget} ps (expected; frame noise only, not the ensemble target)` : `No single-run length is projected for this run (verdict: ${unc.verdict})`}; the matched seed SD ${round(s!, 2)} is ${spreadOverWithin}× this run's corrected SEM, ${spreadOverWithin! >= 2 ? "so seed spread, not frame noise, limits the estimate" : spreadOverWithin! >= 1.2 ? "so seed spread and frame noise are comparable" : "so the seed spread is not distinguishable from this run's frame noise"}.`;
  const assumptions = [
...
export function forkExperiment(m: Manifest, idx: IndexEntry[], opts: { kind: ForkKind; treatment?: { key: string; value: string }; question?: string; stages?: string[] }) {
  ...
  if (opts.kind === "replicate") {
    ...
    return { fork_id: id, kind: opts.kind, parent: m.id, tests: "independent replication — once executed and extracted, an independent-seed run of the same protocol joins the matched same-engine, same-length stratum; its spread becomes reportable once at least three runs exist",
