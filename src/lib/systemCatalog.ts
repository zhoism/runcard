...
export function describeSystem(title: string, ligand: string): { name: string; sentence: string } | null {
  ...
  return { name: `${s.protein} · ${lig}`, sentence: `${s.protein}, ${s.protein_note} (PDB ${pdb}), with ${lig} bound.` };
}
