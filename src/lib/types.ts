...
export interface IndexEntry { id: string; title: string; /** handle of whoever published the card (public/runs/owners.json) */ owner?: string; ligand: string; protein_atoms: number; production_ps: number; delta_g: number; plip: boolean; engine: string; system: SystemKey; protocol?: string; seed?: number | null; parent?: string | null; fork?: { kind: string; seed?: string; complete?: boolean } | null }

/** A profile: who published a set of cards. Site metadata, not a login — there are no accounts. */
export interface Owner { name: string; bio?: string; where?: string }
export interface Owners { default: string; profiles: Record<string, Owner> }
