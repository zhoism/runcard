...
interface State { route: string; proposals: Proposal[]; calls: ToolCall[]; investigations: Record<string, InvestigationState>; webmcp: "unsupported" | "registered" | "error"; tools: string[]; console: { tool: string; input: string } | null; /** a request from elsewhere on the page to open one stage's proposal thread */ openStage: string | null }
