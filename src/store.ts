...
interface State { route: string; proposals: Proposal[]; calls: ToolCall[]; investigations: Record<string, InvestigationState>; webmcp: "unsupported" | "registered" | "error"; tools: string[]; console: { tool: string; input: string } | null }
let state: State = { route: location.hash.slice(1) || "/", proposals: [], calls: [], investigations: {}, webmcp: "unsupported", tools: [], console: null };
