...
interface State { route: string; proposals: Proposal[]; calls: ToolCall[]; bundle: { name: string; files: Record<string, string> } | null; webmcp: "unsupported" | "registered" | "error"; tools: string[] }
let state: State = { route: location.hash.slice(1) || "/", proposals: [], calls: [], bundle: null, webmcp: "unsupported", tools: [] };
