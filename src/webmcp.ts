...
export const TOOLS: Tool[] = [
  ...
  { name: "get_ensemble", readOnly: true, description: "When do I need variation across independent peers of the same prepared system? Returns run-level ΔG statistics for all peers, peers at least 10 ps, and the seed-only stratum (same engine and length); for children forked from this run use fork_network. It leaves nothing on the page.",
  ...
  { name: "fork_network", readOnly: true, description: "When do I need results from children forked from this run? Returns fork lineage, engines, lengths, ΔG values, cohort spread, and agreement or an observed shift from the parent (status cross_engine_shift when the engines differ, never a significance claim); for independent peers of the same prepared system use get_ensemble. It leaves nothing on the page.",
  ...
  try {
    switch (name) {
      ...
      case "plan_sampling": { const r = out.run_to_run, Lp = out.within_run.expected_length_for_target_ps, T = out.target_uncertainty_kcal;
        const one = Lp != null ? `the within-run frame SEM alone is projected to reach the target at ≈ ${f(Lp, 0)} ps (not the ensemble target)` : `single-run length not projected (${out.within_run.this_run.verdict})`;
        return r.planned_on
