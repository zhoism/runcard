...
import { loadIndex, loadRun, validateStage, validateAll, explainResult, diffRuns, makeProposal, rerunBundle, ensemble, recomputeResult, planSampling, verdictOf, confidenceLadder, forkExperiment } from "./lib/runs";
  ...
  try {
    switch (name) {
      ...
      case "fork_experiment": return out.kind === "extend"
        ...
        : `${out.kind}: seed ${out.seed_policy.split(":")[0]}${out.runs_recommended ? (out.runs_recommended.additional_runs != null ? `, ${out.runs_recommended.additional_runs} more run${out.runs_recommended.additional_runs === 1 ? "" : "s"} ≥ ${out.runs_recommended.min_run_ps} ps recommended for ±${out.runs_recommended.target_uncertainty_kcal}` : `, no run-to-run estimate yet (${out.runs_recommended.now ?? "single run"}); ≥ 3 independent runs ≥ ${out.runs_recommended.min_run_ps} ps needed`) : ""}; next generate_rerun_bundle`;
